...
class BitFieldTest(unittest.TestCase):
    ...
    def test_nonint_types(self):
        ...
        self.assertEqual(
            result, (TypeError, "bit fields not allowed for type c_void_p")
        )

        if c_int != c_long:
            result = self.fail_fields(("a", POINTER(c_int), 1))
            self.assertEqual(
                result, (TypeError, "bit fields not allowed for type LP_c_int")
            )
    ...
    def test_single_bitfield_size(self):
        for c_typ in int_types:
            ...
            self.assertEqual(
                result, (ValueError, "number of bits invalid for bit field")
            )
