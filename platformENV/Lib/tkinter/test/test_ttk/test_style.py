...
class StyleTest(AbstractTkTest, unittest.TestCase):
    ...
    def test_lookup(self):
        ...
        self.assertEqual(
            style.lookup("TButton", "background", ["active", "background"]), "blue"
        )
        self.assertEqual(
            style.lookup("TButton", "optionnotdefined", default="iknewit"), "iknewit"
        )
        ...
        for theme in self.style.theme_names():
            ...
            for name in CLASS_NAMES:
                ...
                with self.subTest(theme=theme, name=name):
                    ...
                    if theme in ("vista", "xpnative") and sys.getwindowsversion()[
                        :2
                    ] == (6, 1):
            ...
            for name in CLASS_NAMES:
                ...
                with self.subTest(theme=theme, name=name):
                    ...
                    if (
                        theme == "alt"
                        and name == "."
                        and get_tk_patchlevel(self.root) < (8, 6, 1)
                    ):
    ...
    def test_element_create_image(self):
        ...
        img1 = tkinter.BitmapImage(
            master=self.root, file=imgfile, foreground="yellow", background="blue"
        )
        img2 = tkinter.BitmapImage(
            master=self.root, file=imgfile, foreground="blue", background="yellow"
        )
        img3 = tkinter.BitmapImage(
            master=self.root, file=imgfile, foreground="white", background="black"
        )
    ...
    def test_theme_create_image(self):
        ...
        self.assertEqual(style.lookup("TestWidget", "foreground", ["active"]), "yellow")
        self.assertEqual(
            style.lookup("TestWidget", "foreground", ["active", "pressed"]), "red"
        )
        self.assertEqual(
            style.lookup("TestWidget", "foreground", ["active", "disabled"]), "blue"
        )
        self.assertEqual(
