...
class Sheet:
    ...
    def selectcells(self, x1, y1, x2, y2):
        ...
        if y1 > y2:
            y1, y2 = y2, y1
        return [(x, y) for x, y in self.cells if x1 <= x <= x2 and y1 <= y <= y2]
...
class SheetParser:
    ...
    def end_value(self, text):
        if isinstance(self.value, BaseCell):
            ...
            self.cell = StringCell(self.value, self.fmt or "%s", self.alignment or LEFT)
        else:
            self.cell = NumericCell(
                self.value, self.fmt or "%s", self.alignment or RIGHT
            )

    def end_formula(self, text):
        self.cell = FormulaCell(text, self.fmt or "%s", self.alignment or RIGHT)
...
class StringCell(BaseCell):
    ...
    def xml(self):
        ...
        return s % (align2xml[self.alignment], self.fmt, escape(self.text))
...
class SheetGUI:
    ...
    def __init__(self, filename="sheet1.xml", rows=10, columns=5):
        ...
        self.beacon = Tk.Label(self.root, text="A1", font=("helvetica", 16, "bold"))
    ...
    def makegrid(self, rows, columns):
        ...
        for x in range(1, columns + 1):
            for y in range(1, rows + 1):
                cell = Tk.Label(self.cellgrid, relief="sunken", bg="white", fg="black")
